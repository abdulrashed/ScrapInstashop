...
function scrapeAndExport() {
    ...
    const data = [
        ['Product Title', 'Packaging', 'Price']
    ];

    productElements.forEach(el => {
        const title = el.querySelector('.product-title') ? el.querySelector('.product-title').textContent.replace(/\s+/g, ' ').trim() || '' : '';
        const packaging = el.querySelector('.product-packaging-string') ? el.querySelector('.product-packaging-string').textContent.replace(/\s+/g, ' ').trim() || '' : '';
    ...
    XLSX.writeFile(wb, "html_products.xlsx");
}
